...
ELASTIC_INDEX_DEFAULT = os.getenv('ELASTIC_INDEX_DEFAULT', 'proy_amug')
...
def gestor_usuarios():
    """Página de gestión de usuarios (requiere login + permiso admin_usuarios)"""
...
def actualizar_usuario():
    ...
    try:
        ...
        # Si cambió el nombre, revisar duplicados
...
def gestor_elastic():
    """Página de gestión de ElasticSearch (requiere login + permiso admin_elastic)"""
    if not session.get('logged_in'):
        flash('Por favor, inicia sesión para acceder a esta página', 'warning')
        return redirect(url_for('login'))
...
def cargar_doc_elastic():
    """Página de carga de documentos a ElasticSearch (requiere login + permiso admin_data_elastic)"""
    if not session.get('logged_in'):
        flash('Por favor, inicia sesión para acceder a esta página', 'warning')
        return redirect(url_for('login'))
...
def cargar_documentos_elastic():
    ...
    try:
        ...
        if metodo == 'zip':
            ...
            # Procesar archivos con PLN (placeholder)
            for archivo in archivos:
                ruta = archivo.get('ruta')
                ...
                try:
                    resumen = ""     # TODO: reemplazar por PLN real
                    entidades = ""   # TODO: reemplazar por PLN real
                    temas = []       # TODO: reemplazar por PLN real
                    ...
                    documento = {
                        ...
                        'entidades': entidades,
                        'temas': [
                            {'palabra': palabra, 'relevancia': relevancia}
                            for palabra, relevancia in temas
                        ]
                    }
